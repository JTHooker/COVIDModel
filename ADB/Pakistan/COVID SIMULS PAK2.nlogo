...
;; sets color of patches to black
 ...
 ;; set up people in the environment and allocates characteristics to them
  ask n-of Population patches with [ pcolor = black ]
    [ sprout-simuls 1
      [ set size 2 set shape "dot" set color 85 set householdUnit random 390 set agerange 95  set timenow 0 set IncubationPd int ownIncubationPeriod set InICU 0 set anxiety 0 set sensitivity random-float 1 set R 0
...
;;  ask simuls with [ agerange > 80 ] [ if 95 > random 100 [ set householdUnit random 360 + 720 ] ] ;; allocated older adults 80+  to household Units that don't include young children or teenagers
 ;; ask simuls with [ agerange > 18 and agerange <= 60 ] [ if 95 > random 100 [ if count simuls with [ householdUnit = [ householdUnit ] of myself ] > 2 [
  ...
  ask simuls with [ agerange = 15 and agerange = 5 and studentFlag != 1 ] [ set householdUnit [ householdUnit ] of one-of simuls with [ householdUnit <= 375 and agerange > ([ agerange ] of myself + 15 ) ] ]  ;; Identifies students
...
;    if ticks < seedticks and scalephase = 2 [ ask n-of int .8 simuls with [ color = 85 ] [ set color red set timenow int Case_reporting_delay - 1 set Essentialworker random 100  set unDetectedFlag  0 ]]
    ...
    if ticks < seedticks and scalephase = 0 [ ask n-of int ((1650 * (1.05 ^ (ticks + 15))) )  simuls with [ color = 85 ] [ set color red set timenow int Case_reporting_delay - 1 set Essentialworker random 100 set unDetectedFlag  0 ]]
    if ticks < seedticks and scalephase = 1 [ ask n-of int ((1650 * (1.05 ^ (ticks + 15))) / 10  ) simuls with [ color = 85 ] [ set color red set timenow int Case_reporting_delay - 1 set Essentialworker random 100  set unDetectedFlag  0 ]]
    if ticks < seedticks and scalephase = 2 [ ask n-of int ((1650 * (1.05 ^ (ticks + 15))) / 100 ) simuls with [ color = 85 ] [ set color red set timenow int Case_reporting_delay - 1 set Essentialworker random 100  set unDetectedFlag  0 ]]
...
set App_uptake App_Uptake + random-normal 0 4</setup>
  ...
  <experiment name="Pakistan Unmitigated" repetitions="500" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count turtles</metric>
  ...
  <experiment name="Pakistan Self Govern" repetitions="500" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count turtles</metric>
  ...
  <experiment name="Pakistan Self Govern * 186" repetitions="500" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="150"/>
    <metric>count turtles</metric>
