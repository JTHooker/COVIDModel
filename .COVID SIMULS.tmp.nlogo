...
To Unlock
  if PolicyTriggerOn = true and LockDown_Off = true and ticks >= Triggerday and  timeLockdownOff - ticks ) > PPA * decay_limit ) [
    set Proportion_People_Avoid PPA - ( PPA / ( timeLockdownOff - ticks ))  ]
  if PolicyTriggerOn = true and LockDown_Off = true and ticks >= Triggerday and ( PTA / ( timeLockdownOff - ticks ) > PTA * decay_limit ) [
