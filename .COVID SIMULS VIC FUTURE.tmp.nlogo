...
;; sets color of patches to black
  ...
  ask n-of Population patches with [ pcolor = black ]
    [ sprout-simuls 1
      [ set size 2 set shape "dot" set color 85 set householdUnit random 1000 set agerange 95  set timenow 0 set IncubationPd int ownIncubationPeriod set InICU 0 set anxiety 0 set sensitivity random-float 1 set R 0
        ...
        set ownMaskEfficacy maskWearEfficacy * 33  ;; assigng mask efficacy to individuals around a distribution with median 75% or 75% x 1/3 if 33 as per DHHS request based on Burnett Institute #s
 ...
 ;; ask n-of 90 simuls with [ color = 85 ] [  set reported 1 set color yellow set timenow 0 set health (100 - agerange ) set inICU 0 set requireICU 0  ] ;; this is for the MJA paper
  ...
  ask simuls [
    if any? other simuls-here with [ color = red ]  and 50 > random 100 [ set color yellow ]
...
to scaleup ;; this function scales up the simulation over 5 phases at base 10 to enable a small and large-scale understanding of dynamics. It enables the fine-grained analysis in early stages
  ...
  ifelse scale = true and ( count simuls with [ color = red ] )  >= 250 and scalePhase >= 0 and scalePhase < 4 and count simuls * 1000 < Total_Population and days > 0  [ ;;;+ ( count simuls with [ color = yellow ] )
    set scalephase scalephase + 1 ask n-of ( count simuls with [ color = red ] * .9 ) simuls with [ color = red ] [ set size 2 set shape "dot" set color 85 ;;resethealth calculateincomeperday calculateexpenditureperday
...
to seedCases ;; set up to take the pre-intervention growth pre ******August 25th ********* and use it to seed new cases in the next week - must be updated each day
    if ticks <= seedticks and scalephase = 0 [ ask n-of int (331 * (.92589 ^ (ticks + 15)))  simuls with [ color = 85 ] [ set color red set timenow 6 set Essentialworker random 100 set shape "star" ]]
    if ticks <= seedticks and scalephase = 1 [ ask n-of int ((331 * (.92589 ^ (ticks + 15))) / 10  ) simuls with [ color = 85 ] [ set color red set timenow 6 set Essentialworker random 100 set shape "star"  ]]
    if ticks <= seedticks and scalephase = 2 [ ask n-of int ((331 * (.92589 ^ (ticks + 15))) / 100 ) simuls with [ color = 85 ] [ set color red set timenow 6 set Essentialworker random 100 set shape "star"  ]]
...
;    if ticks > 0 and ticks >= resetdate [ set resetdate (ticks + 7) ]
  ...
  ;; new section
    ...
    if stage = 3 and casesinperiod14 >= threetofour and ticks = resetdate [ set stage 4 set resetdate (ticks + JudgeDay4) ]
    ;;if stage = 4 and casesinperiod7 <= fourtothree and ticks = resetdate [ set stage 3 set resetdate (ticks + JudgeDay3)]
