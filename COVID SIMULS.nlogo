...
to setup
  ...
  ask medresources [ set color white set shape "Health care" set size 10 set capacity Bed_Capacity moveaway set xcor 40 set ycor 0 ]
  ask medresources [ ask patches in-radius Capacity [ set pcolor white ] ]
  ask n-of Toilet_Rolls patches [ sprout-resources 1 ]
  ask resources [ set color white set shape "Bog Roll2" set size 5 set volume one-of [ 2.5 5 7.5 10 ]  moveaway resize set xcor -40 set ycor one-of [ -30 -10 10 30 ] resetlanding ]
  ask n-of Population patches with [ pcolor = black ]
    [ sprout-simuls 1
      [ set size 5 set shape "Worker1" set color 85 set health (random 100) set timenow 0 set pace Speed set InICU 0 set fear 0 set sensitivity random-float 1]
    ]
...
to go
  ask simuls [ move avoid if remainder ticks 2 = 0 [ set shape "Worker1" ]
  if remainder ticks 2 = 1 [ set shape "Worker2" ] recover settime karkit move avoid isolation reinfect createfear gatherreseources treat ] ;
...
to avoid
  if avoidbutton = true  [
    ifelse any? other simuls-on patch-at-heading-and-distance heading forwarddistance [ set pace 0 fd pace set heading heading + random 90 - random 90  ]
...
to superSpread
  if count simuls with [ color = red ] > 0 and isolate = false [  if Superspreaders > random 1000 [ ask one-of simuls with [ color = red ] [move-to one-of patches with [ pcolor = black ]]]]
end

to PossiblyDie
  if InICU = 0 and Seriousness_of_infection / Infectious_Period > random 100 [ set health health - Seriousness_of_Infection ]
  if InICU = 1 and Seriousness_of_infection / Infectious_Period > random 100 [ set health health - Seriousness_of_Infection / 2 ]
end
